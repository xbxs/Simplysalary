...
public class VacateAction extends BaseAction {
	...
	public void queryVacateByTimeAndId(){
		try{
			...
			if(requestCode.equals("section_Vacate")){
				//得到总请假
					for(int i = 0;i< jsonarray.size();i++){
						List<Vacate> list = vacatedao.queryVacateByTimeAndId(btime, etime, jsonarray.getString(i), section);
						double allterm = 0;
			...
			//查询个人请假
				...
				System.out.println("section:-----------------------------------------"+section+" user "+user.toString());
				List<Vacate> list = vacatedao.queryVacateByTimeAndId(btime, etime, u_phone, section);
				commonresponse.setResCode("0");
	...
	//插入请假
		public void addVacate(){
			...
			Vacate vacate = new Vacate(1,u_phone,v_rtime,v_term,v_shift,user.getU_section(),v_reason,user.getU_ower(),v_status,v_btime,v_etime,v_type);
			CommonResponse commonresponse = new CommonResponse();
...
//			Vacate vacate = new Vacate(1,u_phone,v_rtime,v_term,v_shift,v_section,v_reason,v_tuser,v_status,v_btime,v_etime,v_type);
			CommonResponse commonresponse = new CommonResponse();
